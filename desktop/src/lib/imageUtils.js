...
export const convertImagePlaceholders = (html, apiBaseUrl) => {
  ...
  return html.replace(/\[IMAGE:([^\]]+)\]/gi, (match, imagePath) => {
    ...
    return `<img src="${imageUrl}" style="max-width: 100%; height: auto; border-radius: 8px; margin: 8px 0; display: block;" alt="Question image" />`;
  });
};

// For React components that use dangerouslySetInnerHTML
export const processQuestionHTML = (html, apiBaseUrl) => {
  let processed = convertImagePlaceholders(html, apiBaseUrl);
  
  // Ensure the HTML is safe and images load properly
  return processed;
};
